<template>
  <div class="between">
    ...
    >
      ...
      <slot name="form-item-task-permissionFlag" :model="form" field="permissionFlag">
        <el-form-item label="权限标识">
          ...
          <el-select
            ...
            :disabled="disabled"
            :clearable="!disabled"
            @click.native="initUser"
            popper-class="dialogSelect"
            :popper-append-to-body="false"
            @clear="() => event.preventDefault()"
            @remove-tag="() => event.preventDefault()"
          >
            <el-option
          ...
          <!-- <div
            class="flex flex-wrap gap-2 w-full h-full overflow-hidden items-center border-rd-4 px-7 py-2"
            style="box-shadow: 0 0 0 1px var(--el-input-border-color, var(--el-border-color)) inset"
            v-else
            @click="initUser"
          >
            <el-tag
              v-for="(tag, index) in form.permissionFlag"
              :key="tag"
              :closable="!disabled"
              type="info"
              @close="form.permissionFlag.splice(index, 1)"
            >
              {{ tag }}
            </el-tag>
          </div> -->
      ...
      <slot name="form-item-task-listenerType" :model="form" field="listenerType">
        <el-form-item label="监听器类型">
          <el-select v-model="form.listenerType" multiple collapse-tags>
    ...
    <!-- 权限标识：会签票签选择用户 -->
    <el-dialog title="用户选择" v-model="userVisible" width="80%" append-to-body>
      <selectUser
        v-model:selectUser="form.permissionFlag"
        v-model:userVisible="userVisible"
...
function getPermissionFlag() {
  if (form.value.permissionFlag && !Array.isArray(form.value.permissionFlag)) {
    form.value.permissionFlag = form.value.permissionFlag.split(',');
  }
  if (form.value.listenerType && !Array.isArray(form.value.listenerType)) {
      ...
      deptTreeSelect().then((response) => {
        let groupOption = {
          ...
          options: response.data.map((item) => {
            return {
              value: 'dept:' + item.costCenterCode,
              label: item.fullName,
            };
          }),
        };
