      ...
      <el-col :span="20" :xs="24">
        ...
        >
          ...
          <el-form-item>
            ...
            <el-button type="primary" :disabled="checkedItemList.length === 0" @click="submitForm">
              确 定
            </el-button>
          </el-form-item>
        </el-form>
          ...
          <el-table-column width="50" align="center">
            ...
            <template #default="scope">
              <el-checkbox
                ...
                @change.capture="handleCheck(scope.row)"
              ></el-checkbox>
            </template>
          </el-table-column>
          <el-table-column
...
import { sortByCode } from '@/utils/utils';
...
const loopSort = (data) => {
  if (data && data.length > 0) {
    data.sort(sortByCode('costCenterCode'));
    data.forEach((item) => {
      if (item.children && item.children.length > 0) {
        item.children.sort(sortByCode('costCenterCode'));
      }
    });
    return data;
  } else {
...
function getDeptTree() {
  api.deptTreeSelect().then(({ code, data }) => {
    if (code === 200) {
      const tempArr = loopSort(data);
      deptOptions.value = [
        {
          id: '0',
          costCenterCode: '',
...
function getList() {
  ...
  if (props.type) {
    ...
    params = { ...queryParams.value, ...postParams, deptId: queryParams.value.deptId };
  }
...
function handleNodeClick(data) {
  queryParams.value.deptId = data.costCenterCode;
  handleQuery();
}
/** 搜索按钮操作 */
